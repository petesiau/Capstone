# Global objects
# It will load necessary libraries and the dataset produced earlier and break it into separate smaller NGram dataframes.

library(data.table)

load("NGramData.RData")

dataIn <- fread("finalNGramSorted1.txt", sep = "\t", header=TRUE, data.table = FALSE)
...
source('./func_PredictWord.R')

func_CleanData <- function(x, flag = TRUE) {
